...
function ProductDetail(id) {
  ...
  const [dataSuggestFilter, setDataSuggestFilter] = useState(null);
  ...
  useEffect(() => {
    const filterProductGift = data
      ?.map((item) =>
        item?.gifts?.length > 0 ? item.gifts?.map((gift) => gift) : null
      )
      .filter((item) => item !== null)
      .flat();
    const res = dataSuggest?.filter((el) => {
      return !filterProductGift.includes(el._id);
    });
    setDataSuggestFilter(res);
  }, [dataSuggest]);
  return (
    <div className=" px-1 bg-[#f4f1f4] py-10">
      <div className="md:mx-[10%] bg-white">
        <p className="text-[18px] md:text-[24px] mx-2 font-[700] px-2">
          Sản phẩm
        </p>
        ...
        {productData?.reviews?.length > 0 && (
          <div className="w-full ">
            <div className="relative">
              <p className="  text-[14px] md:text-[18px] font-[600] px-2">
                Đánh giá
              </p>
              <div
